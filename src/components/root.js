...
const TodoComponent = {
 ...
 class root extends Component {
   ...
   this.name=this.name.bind(this);
    ...
    <br></br>
       <div style={m}><input type="text" 
       onChange={this.name}
       id='name' placeholder='Product Id or Name'></input>
        ...
        <table className="table" style={Header}>
          ...
          { this.props.products.map((product, key) => {
            if(this.state.N==(product.name)||this.state.N==(product.id+'')){
  return(
    ...
    <tr key={key}>
      ...
      <td>{product.owner} </td>
      <td>
        { product.purchased!==true&&product.owner!=this.state.account
          ? <button
              name={product.id}
              value={product.price}
              onClick={(event) => {
